...
class _DeleteReportPageState extends State<DeleteReportPage> {
          ...
          child: Column(
            children: [
              ...
              ListTile(
                ...
                subtitle: Text('${widget.report['latitude']}, ${widget.report['longitude']}'),
              ),
              ListTile(
                title: Text('Severity', style: TextStyle(fontWeight: FontWeight.bold)),
  ...
  void _deleteReport() {
    ...
    String decodedUrl = Uri.decodeFull(widget.report['imageUrl']); // Use widget.report to access the report object
    ...
    storageRef.delete().then((_) {
      FirebaseFirestore.instance.collection('reports').doc(widget.report.id).delete().then((_) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Report deleted successfully")));
        ...
        Future.delayed(Duration(seconds: 0), () {
          if (Navigator.canPop(context)) {
            ...
            Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => ViewStatus()));
          }
        });
      }).catchError((error) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Error deleting report: $error")));
